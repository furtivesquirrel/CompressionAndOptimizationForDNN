...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='Convert Yolov3 weights to ONNX',
        description='This script converts YoloV3 darknet weights to ONNX')

    parser.add_argument('--model_path', 
        help='Model\'s path (.weights)',
    ...
    parser.add_argument('--config_path', 
        help='Yolo config path (.cfg)',
        default='../../model_data/Suspect/yolov3-tiny-suspect.cfg')

    parser.add_argument('--img_size', 
        help='Yolo input image size (288, 416, 608)',
        default='416')

    parser.add_argument('--output_onnx_model', 
        help='ONNX model output path (.onnx)',
        default='../../model_data/Suspect/yolov3-tiny-suspect.onnx')

    args = parser.parse_args()

    cfg_file_path = args.config_path
    weights_file_path = args.model_path
    output_file_path = args.output_onnx_model
    yolo_dim = int(args.img_size)
    ...
    builder = GraphBuilderONNX(args.model_path, output_tensor_dims)
