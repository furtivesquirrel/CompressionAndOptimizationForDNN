...
class YOLO(object):
    ...
    _defaults = {
        "model_path": 'yolov3-mask-adaptivity.h5',
        "anchors_path": 'yolo_anchors.txt',
        "classes_path": 'classes_mask2.txt',
        "score" : 0.3,
        "iou" : 0.45,
    ...
    def detect_image_ann(self, image, annotation_path):
        start = timer()
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            bottom = min(image.size[1], np.floor(bottom + 0.5).astype('int32'))
            right = min(image.size[0], np.floor(right + 0.5).astype('int32'))
            print('DETECTION OBJET')
            print(label, (left, top), (right, bottom))

            # enregistrer les annotations dans fichier txt
